...
export default function Welcome() {
  ...
  // Form Modal State
  const [selectedServiceForForm, setSelectedServiceForForm] = useState<{id: string, name: string, icon: any} | null>(null);
    ...
    <SidebarLayout>
      <div className="w-full text-slate-200 overflow-y-auto pb-20 font-sans selection:bg-neon-blue/30 overflow-x-hidden">
        ...
        <div className="w-full max-w-sm flex flex-col gap-3 z-10">
          ...
          >
            <Siren size={20} className="text-red-400 group-hover:scale-110 transition-transform" />
          ...
          >
            <Wrench size={20} className="text-neon-blue group-hover:rotate-12 transition-transform" />
        ...
        <div className="max-w-3xl mx-auto px-6">
          <div className="flex flex-col sm:flex-row items-center justify-between mb-8 text-center sm:text-left gap-4">
             <div>
               ...
               <p className="text-slate-400 text-xs">Seja rápido, dezenas de mecânicos estão de olho nestes contatos.</p>
             </div>
             <button 
                onClick={handleProfessionalAccess}
             ...
             <AnimatePresence>
                {liveCalls.length > 0 ? (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-4 w-full sm:w-auto">
                        ...
                        <div className="flex-1 text-left">
                           <div className="font-bold text-slate-200 capitalize text-sm">{call.serviceRequested}</div>
                  ...
                  <div className="text-center py-10 bg-[#0B101A] rounded-2xl border border-slate-800/50">
                     ...
                     <p className="text-xs text-slate-500 font-medium tracking-widest uppercase">Aguardando novos chamados...</p>
                  </div>
                )}
             </AnimatePresence>
          </div>
      ...
      <section id="categorias" className="px-6 pt-12 pb-12 max-w-xl mx-auto">
        <div className="text-center mb-8">
          <h2 className="text-[10px] uppercase tracking-widest text-slate-500 font-bold mb-2">Categorias de Serviço</h2>
          <h3 className="text-2xl font-black text-white leading-tight">O QUE PRECISAR, A GENTE ENCONTRA</h3>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          {CATEGORY_TAGS.map((tag) => (
             ...
             >
               <tag.icon size={32} className={`${tag.color} group-hover:scale-110 transition-transform`} />
               <span className="font-bold text-slate-200 uppercase tracking-widest text-xs">{tag.label}</span>
             </button>
          ))}
        </div>
      </section>
           ...
           <div className="bg-[#111827] w-full max-w-md rounded-t-3xl sm:rounded-3xl border border-slate-800 shadow-2xl flex flex-col max-h-[90vh] animate-in slide-in-from-bottom-10 sm:slide-in-from-bottom-4 duration-300">
              ...
              <div className="flex justify-between items-center p-5 border-b border-slate-800/80">
                 <div className="flex items-center gap-3">
                    <h2 className="text-lg font-bold text-white uppercase tracking-wider">Novo Pedido</h2>
                 </div>
                 <button onClick={closeForm} className="p-2 bg-slate-800/50 hover:bg-slate-700 rounded-full text-slate-400 transition-colors">
                    <X size={20} />
                 </button>
              </div>
              ...
              <div className="p-5 overflow-y-auto">
                 <div className="mb-6 flex items-center justify-center gap-2 bg-slate-800/60 border border-slate-700/50 px-4 py-3 rounded-lg text-sm font-semibold text-white">
                    {selectedServiceForForm.icon && <selectedServiceForForm.icon size={18} />}
                    {selectedServiceForForm.name}
                 </div>

                 <form id="requestForm" onSubmit={submitRequest} className="space-y-4">
                    <div>
                    ...
                    <div>
                       <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Veículo</label>
                       <input 
                          type="text" 
                          required
                          value={vehicle}
                    ...
                    <div>
                       <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Descreva o Problema</label>
                       <textarea 
                          required
                    ...
                    <div>
                       <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Sua Região em Brasília</label>
                       <input 
                          type="text" 
                          required
                          value={region}
